...
import com.articlesystem.Utils.LayUIJsonResult;
import com.articlesystem.Utils.MyUtils;
import com.articlesystem.entity.FileInfo;
import com.articlesystem.entity.UploadImgInfo;
...
@WebServlet(urlPatterns = "/layui/upload/img")
public class LayUIEditFileUploadServlet extends HttpServlet {
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
      throws ServletException, IOException {
    // 存储返回信息。
    LayUIJsonResult layUIJsonResult = new LayUIJsonResult<UploadImgInfo>();
    ...
    try {
      ...
      UploadImgInfo uploadImgInfo = new UploadImgInfo(fileUrl, "filename");

      layUIJsonResult = layUIJsonResult.success(uploadImgInfo);

    } catch (InvalidFileSuffixException e) {
      e.printStackTrace();
      layUIJsonResult = layUIJsonResult.fail();
    ...
    // 写数据给前端
    //toJSONString = JSON.toJSONString(layUIJsonResult);
    String json = new Gson().toJson(layUIJsonResult);
    MyUtils.JsonResultToWrite(json, response.getWriter());
  }
  ...
  private List<FileInfo> buildFileUploadBeans(List<FileItem> items,
  ...
  /**
   * 根据跟定的文件名构建一个随机的文件名 1. 构建的文件的文件名的扩展名和给定的文件的扩展名一致 2. 获取一个tomcat的虚拟路径,将虚拟路径存放在数据库中，以便于用浏览器访问资源 3.
   * 利用了 Random 和 当前的系统时间构建随机的文件的名字
   *
   * @param fileName
   * @return
   */
  private String getTempFilePath(String fileName) {
    ...
    String filePath =
        TomcatVirtualDirectoty + System.currentTimeMillis() + random.nextInt(100000) + suffix;
    return filePath;
  }
